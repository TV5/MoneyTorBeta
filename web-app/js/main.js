		...
		.ready(
				function() {
					...
					function setreceivablesTotalAmt() {
						var receivablesAmounts = $("#receivablesTable")
								.dataTable().$('tr', {
									"filter" : "applied"
								}).find(':nth-child(3)');
					...
					receivablesTable.buttons(0, null).container().prependTo(
							receivablesTable.table().container());
					...
					$('#receivablesNumEntries').change(
							function() {
								receivablesTable.page.len(
										$('#receivablesNumEntries').val())
										.draw();
							});
					...
					payablesTable.buttons(0, null).container().prependTo(
							payablesTable.table().container());
					...
					function setPayablesTotalAmt() {
						var payablesAmounts = $("#payablesTable").dataTable()
								.$('tr', {
									"filter" : "applied"
								}).find(':nth-child(3)');
						var payablesTotal = 0;
					...
					$('#payablesNumEntries').change(
							function() {
								payablesTable.page.len(
										$('#payablesNumEntries').val()).draw();
							});
...
function validateAccount(errorList, errorDiv, transactorList, or_no, amount,
		transactorType, addMoreBtn, saveBtn, formInputs) {
	errorList.empty();
	if (transactorList == null || or_no == "" || amount == "" || amount < 1
			|| /[^a-zA-Z0-9]/.test(or_no)) {
		errorDiv.show();

		if (transactorList == null) {
			errorList
					.append('<li>Please select a supplier from the list provided.'
							+ ' If you cannot find the supplier you are looking for, please select <b>Create new supplier</b>'
							+ ' to add a new supplier.</li>');
		}

		if (or_no == "") {
			errorList
					.append('<li>Please enter an official receipt number.</li>');
		}

		else if (/[^a-zA-Z0-9]/.test(or_no)) {
			errorList
					.append('<li>Official receipt number must be alphanumeric.</li>');
		}
...
function psaved() {
	...
	validateAccount(errorList, errorDiv, transactorList, or_no, amount,
			transactorType, addMoreBtn, saveBtn, formInputs);
}

/*
 * function epsaved() { var errorList = $('#editPayableErrorList'); var errorDiv =
 * $('#editPayableErrorDiv'); var transactorList = $('#epsupplier_name').val();
 * var or_no = $('#epor_no').val(); var amount = $('#epamount').val(); var
 * transactorType = "supplier"; validateAccount(errorList, errorDiv,
 * transactorList, or_no, amount, transactorType, null, null, null); }
 */
